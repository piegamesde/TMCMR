...
public class Color {
	...
	/**
	 * Return the color resulting from overlaying frontColor over backColor + Front color's RGB should *not* be pre-multiplied by alpha. - Back color must have
	 * RGB components pre-multiplied by alpha. - Resulting color will be pre-multiplied by alpha.
	 */
	...
	public static final int overlay(int color, int frontColor, int repeat) {
		for (int i = 0; i < repeat; ++i)
			color = overlay(color, frontColor);
		return color;
	}

	public static void demultiplyAlpha(int[] color) {
		// TODO optimize
		for (int i = color.length - 1; i >= 0; --i)
			color[i] = Color.demultiplyAlpha(color[i]);
	}
	...
	/**
	 * Multiply 2 colors together by multiplying their R, G, and B components. Alpha is taken from only the first color. This way, the 2nd color acts as a
	 * filter.
	 */
