...
public final class BlockColors {
	...
	/**
	 * Holds the default color for a block and, optionally, a color for specific 'block data' values.
	 */
	...
	public BlockColors(Block[] blocks) {
		assert blocks != null;
	...
	public static BlockColors load(BufferedReader s, String filename) throws IOException {
		...
		while ((line = s.readLine()) != null) {
			...
			if ("default".equals(v[0])) {
				for (int i = 0; i < blocks.length; ++i)
					blocks[i].setBaseColor(color, 0, true);
			} else {
		...
		return new BlockColors(blocks);
	}

	public static BlockColors load(File f) throws IOException {
	...
	public static BlockColors loadDefault() throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(BlockColors.class.getResourceAsStream("block-colors.txt")));
		try {
